...
`tools[]` lists chassis tools by name (see `/etc/chassis/tools-public.json` for what's available). `pi_defaults` toggles Pi built-ins. The seeded `manager` at `/home/agent/manager/` is a good worked example — read its `SYSTEM.md` when you want to model phrasing or structure.

## What you can scaffold

Anything under `/home/agent/` (your own home). You own that tree — no `sudo` needed. Use `bash`/`read`/`write`/`edit` directly.
